...
SNAPSHOT_PATTERN = "combined_scrape_*.csv"
...
def main(paths: PipelineFiles = FILES):
    ...
    prev_name = Path(previous_file).stem.replace("combined_scrape_", "")
    curr_name = Path(current_file).stem.replace("combined_scrape_", "")
