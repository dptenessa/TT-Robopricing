...
def get_country_name_from_iso2(iso2):
    ...
    return ISO2_COUNTRY_NAMES.get(str(iso2).strip().upper())
...
def main():
    ...
    # Final country name is taken from the ISO2 mapping table.
    # If ISO2 is not found in the mapping table, keep the existing country value.
    merged["mapped_country"] = merged["ISO_Code_A2"].apply(get_country_name_from_iso2)
    merged["country"] = merged["mapped_country"].combine_first(merged["country"])
